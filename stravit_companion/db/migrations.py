...
def _backup_database(engine: Engine) -> Path:
    ...
    backup = source.with_suffix(source.suffix + ".pre-pseudonymization.bak")
...
def migrate_leaderboard_snapshots(engine: Engine, identity_hash_key: str) -> None:
    ...
    with engine.begin() as connection:
        ...
        source_rows = connection.execute(
            text(
                "SELECT id, ts, name, rank, distance, elevation, longest, count "
        ...
        for row in source_rows:
            connection.execute(
                text(
                    ...
                    "(id, ts, participant_id, display_name, rank, distance, elevation, "
                ...
                {
                    **row,
                    "participant_id": participant_id(row["name"], identity_hash_key),
